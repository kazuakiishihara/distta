...
class TransMorph_SPTTA(nn.Module):
    def __init__(self, pretrained_path, patch_size=(192, 224, 160)):
        ...
        self.transmorph = regisry.build_model('TransMorph-DS', (192, 224, 160))
...
class TransMorph_SPTTA_ir(nn.Module):
    def __init__(self, pretrained_path, patch_size=(192, 224, 160)):
        super().__init__()
        data_prompt = torch.zeros(1, 768//2, patch_size[0]//32, patch_size[1]//32, patch_size[2]//32)
        self.data_prompt = nn.Parameter(data_prompt)
    ...
    def forward(self, inputs):
        source, tar = inputs # moving, fixed
        perturbation_tar = self.data_adaptor(tar)
        perturbation_source = self.data_adaptor(source)
        adapted_tar = tar + perturbation_tar
        adapted_source = adapted_source + perturbation_source
        x = torch.cat((adapted_source, adapted_tar), dim=1)
        if self.transmorph.if_convskip:
            x_s0 = x.clone()
            x_s1 = self.transmorph.avg_pool(x)
            f4 = self.transmorph.c1(x_s1)
        ...
        feats_m = self.transmorph.transformer(adapted_source)
        feats_f = self.transmorph.transformer(adapted_tar)

        adapted_feats_f, attn_weights = self.prompt2feature(x_q=feats_f[-1], x_kv=self.data_prompt)
        adapted_feats_m, attn_weights = self.prompt2feature(x_q=feats_m[-1], x_kv=self.data_prompt)
        ...
        if self.transmorph.if_transskip:
            ...
            f2 = None
            f3 = None
        x = self.transmorph.up0(torch.cat((adapted_feats_m[-1], adapted_feats_f), dim=1), f1)
        x = self.transmorph.up1(x, f2)
        x = self.transmorph.up2(x, f3)
        x = self.transmorph.up3(x, f4)
        x = self.transmorph.up4(x, f5)
        flow = self.transmorph.reg_head(x)
        out = self.transmorph.spatial_trans(source, flow)
        return out, flow, (perturbation_tar, adapted_tar)
